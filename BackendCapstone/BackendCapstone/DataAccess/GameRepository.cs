...
namespace BackendCapstone.DataAccess
{
    public class GameRepository
    {
        ...
        {
            using var db = new SqlConnection(_connectionString);
            var sqlForPopularGames = @"select top 5 ppg.GameId as Id, g.Name, COUNT(ppg.Id) as Value
                                        ...
                                        order by Value desc";
            var mostPopularGames = db.Query<GamePopular>(sqlForPopularGames);
            List<GamePopular> mostPopularGamesList = mostPopularGames.ToList();
